...
- Knockout helper copy: open a knockout match on `/predictions` (and the single-match form) and confirm the helper text explains entering the final played result, that extra time counts the 120' result, and that a predicted draw is decided by who passes on penalties. Confirm no gambling language.
- Knockout non-draw prediction UX: enter a non-draw knockout score and confirm the qualified team is inferred from the score winner (flag selector auto-updates), and that the prediction saves without manually choosing a qualified team in both the bulk `/predictions` flow and the single-match form.
