...
- Knockout helper copy: open a knockout match on `/predictions` (and the single-match form) and confirm the copy is compact and mobile-friendly — a single lead line ("En eliminatorias, si pronosticás empate, elegí quién clasifica.") with no long paragraph, and the old multi-sentence explanation is gone. Confirm the card is not unnecessarily tall and there is no gambling language. If the 120' clarification is shown, confirm it is short and only appears with the draw selector ("Cuenta el resultado final jugado, incluido alargue.").
- Knockout empty-score UX: open a knockout match with empty score and confirm no qualified team can be chosen (flag buttons hidden/disabled) and a compact neutral helper is shown ("Cargá el resultado para definir quién clasifica.").
- Knockout non-draw prediction UX: enter a non-draw knockout score (e.g. Spain 1-0 United States) and confirm the flag buttons are not shown/selectable and the opposite team cannot be tapped; a compact "Clasifica automáticamente: <equipo>" line appears, and the prediction saves with the inferred qualified team without manually choosing it, in both the bulk `/predictions` flow and the single-match form.
- Knockout draw prediction UX: enter a draw knockout score and confirm the flag/label radio buttons appear and become selectable (not a dropdown); confirm saving without a selection fails with a clear Spanish error and that the chosen team persists once selected.
...
- Live status label: with a match in a live-ish state (`api_status` such as `1H`, `2H`, `HT`, `ET`, `LIVE`), confirm user-facing surfaces (dashboard daily/live cards, `/predictions` status badge) show the simple label "En vivo" and never the raw technical state. Admin/debug/API-health views may still show the raw `api_status`. This is presentation only — stored `api_status` is unchanged.
...
- API-Football stage detection: using a fake/local snapshot only, sync fixtures whose `league.round` covers Group Stage, Round of 32, Round of 16, Quarter-finals, Semi-finals, 3rd Place Final, and Final; confirm each maps to the expected `matches.stage` and that only the knockout stages report `requiresQualifiedTeamPrediction()` true while Group Stage reports false.
- API-Football unknown round handling: using a fake/local snapshot only, sync a fixture with an unrecognized `league.round` label and confirm `stage` is left null, the raw `round` is preserved, the command prints an `Unmapped API round labels:` warning, and the label is recorded under `metadata.unknown_rounds` in the sync log (no silent knockout mis-mapping).
...
- Finished-match consistency checker: run `php artisan prode:check-finished-matches`. It is read-only and safe to run in production/live (it never modifies data). Read the `Summary:` line (finished matches checked, API-finished mismatches, missing scores, missing knockout winners, unscored finished predictions, finished matches with predictions but zero scored) and the per-issue detail rows (local match id, API fixture id, teams, API status, local status, score, winner, issue code). It exits `0` when there are no critical inconsistencies and non-zero when any are found. Critical issue codes: `api_finished_local_not_finished`, `finished_missing_score`, `knockout_finished_missing_winner`, `finished_unscored_predictions`, `finished_predictions_none_scored`, `non_draw_winner_mismatch`, `group_draw_has_winner`, `knockout_winner_not_in_teams`. The `api_live_local_finished` code is a non-critical warning (does not fail the run). After fake/local settlement in staging, confirm a clean dataset exits `0` with no issue rows. The command does not call API-Football and prints no user emails or secrets.
