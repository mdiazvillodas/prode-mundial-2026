...
**Follow-up (done):** Tightened the selector UX and live status copy after iPhone 14 Pro Max staging QA. The `<x-knockout-qualified-selector>` is now score-driven via `data-qualified-state` (`empty`/`auto`/`draw`), computed server-side from the saved/old score and kept in sync by `knockout-inference.blade.php`: empty score hides the flag buttons (compact neutral helper), a non-draw score hides the buttons and shows a compact "Clasifica automáticamente: <equipo>" line (the opposite team can no longer be selected, and the inferred team is still submitted), and only a draw reveals the selectable flag buttons. The long explanatory paragraph was replaced with one short lead line plus a short optional 120' clarification. Separately, user-facing live status now shows "En vivo" instead of raw `api_status` (`1H`/`2H`/…): centralized in `LiveDashboardDataService` (`status_label`/`dailyStatusLabel()`) and fixed in the `/predictions` status badge; stored `api_status` and API sync are unchanged, and admin/debug/API-health views still show raw status. New focused tests cover the empty/non-draw/draw selector states, the removed long copy, and the live-label mapping. No scoring, settlement, API sync, winner-resolution, schema, or leaderboard changes.
